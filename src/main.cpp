...
int main() {
  ...
  for (double v = 0; v < 50; v += 5) {
    ...
    std::string path = "/home/xzc/MRI-Simulator-cpp/result/flow_Mz_128_";
    std::ostringstream oss;
    oss << v;
    string v_str = oss.str();
    path = path + v_str + "/";
    string cmd = "mkdir " + path;
    std::system(cmd.data());
    // std::string save_path =
    // "/home/xzc/MRI-Simulator-cpp/result/flow_Mz_128/";
    fl_program.save_mat(path);
  }
