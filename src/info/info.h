...
struct pool_info {
  ...
  pool_info(double fov, double delta, double z0, double HR, double flow_speed,
            double bandwidth, double TR, int num_vassels);
  void T_info_generate(double T1_vassel, double T2_vassel, double T1_tissue,
                       double T2_tissue);
};
...
public:
  ...
  Voxel();
  void initialize(double T[], bool flow, Vector3d position, Vector3d M,
                  int num_protons, int index_vassel);
